...
function createUser(overrides = {}) {
  return {
    ...
    email: overrides.email ?? 'user@qq.com',
...
test('registerWithEmail 会创建用户、哈希密码并签发 token', async () => {
  ...
  assert.equal(result.user.email, 'new@qq.com');
...
test('registerWithEmail 遇到重复邮箱会抛出业务错误', async () => {
  const context = createTestContext({
    users: [createUser({ email: 'exists@qq.com' })],
  });
...
test('registerWithEmail 不支持白名单外邮箱后缀', async () => {
  const context = createTestContext();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.code, 'UNSUPPORTED_EMAIL_DOMAIN');
      return true;
    },
  );
});
...
test('loginWithEmail 白名单外邮箱后缀返回统一鉴权错误', async () => {
  const context = createTestContext({
    users: [createUser({ email: 'user@qq.com' })],
  });
...
test('getCurrentUser 会返回当前用户资料，缺失时抛出 404', async () => {
  ...
  assert.equal(user.email, 'user@qq.com');
...
test('loginWithOAuth 首次登录会创建用户并绑定 OAuth 账号', async () => {
  ...
  const result = await context.service.loginWithOAuth({
    ...
    email: 'oauth-new@qq.com',
    name: 'OAuth User',
  });

  assert.equal(result.user.email, 'oauth-new@qq.com');
...
test('loginWithOAuth 会将同邮箱 OAuth 账号并入已存在用户', async () => {
  const context = createTestContext({
    users: [createUser({ id: 'user_existing', email: 'merge@qq.com' })],
  });
